        ...
        <div className="flex-1 overflow-auto p-8 space-y-8 max-w-7xl mx-auto w-full">
          ...
          <section className="bg-card border border-border rounded-2xl p-8 shadow-2xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <MetricItem label="No Prazo (> 7 dias)" value={metrics.statusCounts['No Prazo']} pct={getPercent(metrics.statusCounts['No Prazo'])} color="bg-chart-3" />
              <MetricItem label="Arquivado / Encerrado" value={metrics.statusCounts.Arquivado} pct={getPercent(metrics.statusCounts.Arquivado)} color="bg-muted-foreground" />
            </div>

            <div className="h-4 w-full bg-secondary rounded-full flex overflow-hidden">
              ...
              <div style={{ width: `${getPercent(metrics.statusCounts['No Prazo'])}%` }} className="bg-chart-3 h-full transition-all duration-1000" />
              <div style={{ width: `${getPercent(metrics.statusCounts.Arquivado + metrics.statusCounts['Sem Prazo'])}%` }} className="bg-muted-foreground/30 h-full transition-all duration-1000" />
            </div>
          </section>
            ...
            <section className="bg-card border border-border rounded-2xl p-8">
              ...
              <div className="space-y-6">
                {metrics.topTribunals.length > 0 ? metrics.topTribunals.map(([name, count]) => (
                  ...
                  <p className="text-center text-sm text-muted-foreground py-10">Waiting for ingested data...</p>
                )}
              </div>
            </section>
  ...
  return (
    <div className="p-4 bg-secondary/20 border border-border rounded-xl">
      ...
      <div className="flex items-end gap-2">
        ...
        <span className={cn("text-[10px] font-bold mb-1 px-1.5 py-0.5 rounded text-white", color)}>{pct}%</span>
      </div>
    </div>
  );
}
