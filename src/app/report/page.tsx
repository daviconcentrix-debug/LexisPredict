...
import { getStoredCases, getStoredNotes } from '@/lib/server-db';
import { LegalCase, CaseNote } from '@/lib/case-logic';
import { Button } from '@/components/ui/button';
import { Printer, ArrowLeft, Activity, FileText, MessageSquare, ShieldCheck } from 'lucide-react';
import Link from 'next/link';

/**
 * ENGINE DE RELATÓRIO UNIFICADO LEXISPREDICT
 * Consolidado de Intelligence Unit, Analytics Hub e Notes Board
 */
...
export default function UnifiedReport() {
  ...
  // Nome do Usuário para Atribuição de Auditoria no Documento
  const userName = "Davi Alves";
  const userRole = "Análise de Processos Jurídicos";
  ...
  return (
    <div className="min-h-screen bg-white text-black p-8 md:p-12 max-w-5xl mx-auto print:p-0">
      ...
      <header className="border-b-4 border-black pb-6 mb-10">
        ...
        <div className="flex justify-between items-end mt-8">
          <div className="flex gap-2">
            ...
            <span className="text-[10px] border-2 border-black px-3 py-1 font-bold uppercase flex items-center gap-1">
              <ShieldCheck size={10} /> Confidencial & Auditado
            </span>
          </div>
      ...
      {/* SECTION 1: INDICATORS (ANALYTICS HUB) */}
      ...
      {/* SECTION 2: PRIORITY QUEUE (INTELLIGENCE UNIT) */}
      <section className="mb-12">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-1.5 bg-black text-white rounded">
            <FileText size={18} />
      ...
      {/* SECTION 3: NOTES & UPDATES (KEEP BOARD) */}
      <section className="mb-12 break-inside-avoid">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-1.5 bg-black text-white rounded">
            <MessageSquare size={18} />
          </div>
          <h2 className="text-lg font-extrabold uppercase tracking-tight">3. Atualizações e Anotações Estratégicas</h2>
        </div>
        {notes.length > 0 ? (
      ...
      <footer className="mt-20 pt-8 border-t-2 border-gray-100 text-center">
        <p className="text-[9px] text-gray-400 uppercase tracking-[0.3em] font-bold">
          Gerado via LexisPredict AI CRM — Extraído por {userName}
        </p>
      </footer>
    </div>
  );
