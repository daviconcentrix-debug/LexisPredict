...
import { LegalCase, processarCaso, cn } from '@/lib/case-logic';
...
export default function ImportPage() {
  ...
  const parseFile = async (file: File) => {
    ...
    reader.onload = async (event) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        // Basic CSV parsing for simple data
        ...
        if (i % 20 === 0 || i === total) {
          ...
          await new Promise(r => setTimeout(r, 10)); // UI smoothness
        }
      }
  ...
  const commitToStorage = () => {
    ...
    toast({
      title: "Sucesso!",
      description: `${preview.length} processos foram migrados para a base local.`,
    });
                      ...
                      {preview.slice(0, 5).map((c, i) => (
                        <div key={i} className="flex justify-between items-center py-2 border-b border-border/50 last:border-0">
                          ...
                          <div className="flex items-center gap-3">
                            <Badge variant="outline" className="text-[10px]">{c.tribunal}</Badge>
