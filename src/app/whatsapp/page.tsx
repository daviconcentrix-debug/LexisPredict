...
export default function WhatsAppHub() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error('[Gabinete] WhatsApp Hub Sync Fail');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerateAI = async (agent: 'legal' | 'comercial' | 'financeiro') => {
    ...
    const systemContext = `Você é o Setor Processual da Get Assessoria Financeira.
    CLIENTE: ${selectedContact.nome}
    PROTOCOLO: ${selectedContact.protocolo}
    STATUS: ${selectedContact.status}
    TRIBUNAL: ${selectedContact.tribunal}
    OBSERVAÇÕES DO GABINETE: ${contextNotes}
    USUÁRIO LOGADO (ASSINATURA): ${userSignature}
    
    USE OS SCRIPTS ABAIXO COMO GUIA DE ESTILO:
    ${JSON.stringify(SCRIPTS_GABINETE)}
    
    INSTRUÇÃO: Gere uma mensagem de WhatsApp profissional baseada no contexto e nos modelos da empresa.`;

    const prompts = {
      legal: `Redija uma atualização jurídica sobre o status '${selectedContact.status}'.`,
      comercial: `Redija uma mensagem de acompanhamento cordial e atenciosa.`,
      financeiro: `Redija uma mensagem sobre custas judiciais (Cláusula 3.2), sendo firme mas educado.`
    };
    ...
    try {
      ...
      if (res && res.resposta && !res.error) {
        ...
        toast({ title: "Redação Concluída" });
      } else {
        toast({ title: "Erro na IA", description: "O motor falhou. Tente trocar de motor ou verifique a conexão.", variant: "destructive" });
      }
    } catch (error: any) {
      toast({ title: "Erro de Conexão Neural", variant: "destructive" });
    } finally {
  ...
  const handleGenerateFromHistory = async () => {
    ...
    TAREFA: Resuma os fatos de forma simples para o WhatsApp do cliente. Explique o que aconteceu por último e os próximos passos.`;
    ...
    try {
      ...
      if (res && res.resposta && !res.error) {
        setAiResponse(res.resposta);
        toast({ title: "Análise Forense Pronta" });
      } else {
        toast({ title: "Erro na IA", description: "Falha ao processar histórico do tribunal.", variant: "destructive" });
      }
    } catch (error: any) {
      toast({ title: "Erro de Processamento", variant: "destructive" });
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  const handleSendAPI = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: "Mensagem Entregue via Evolution API" });
      } else {
        toast({ title: "Falha na Entrega", description: result.message, variant: "destructive" });
      }
    } catch (e: any) {
  ...
  return (
    <div className="flex h-screen bg-[#f3f2f2] font-sans text-black relative z-10 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        <header className="h-auto lg:h-16 border-b border-[#dddbda] bg-white flex flex-col lg:flex-row lg:items-center justify-between px-6 py-4 lg:py-0 shrink-0 z-40 gap-4">
          ...
          <div className="flex items-center gap-3">
             <div className="flex flex-col items-end mr-2">
               <span className="text-[8px] font-black uppercase text-black/40 mb-0.5">Seletor de Núcleo Neural</span>
            ...
            <div className="p-4 border-b-2 border-black bg-[#f8f9fb]">
              <div className="relative">
                ...
                <Input 
                  ...
                  className="pl-10 border-black border-2 h-10 text-[10px] font-black text-black uppercase bg-white rounded-none"
                />
              </div>
            </div>
            <ScrollArea className="flex-1">
              <div className="divide-y-2 divide-black/5">
                {contacts.map((contact) => (
                  ...
                  >
                    <div className={cn("w-10 h-10 border-2 border-black flex items-center justify-center shrink-0", selectedContact?.id === contact.id ? "bg-white" : "bg-[#f3f2f2]")}>
                          ...
                          <Dialog>
                            <DialogTrigger asChild>
                               <Button variant="outline" size="sm" className="h-8 border-white/20 text-white bg-white/10 hover:bg-white/20 text-[9px] font-black uppercase rounded-none">
                                 <BookOpen size={12} className="mr-2" /> Modelos Oficiais
                               </Button>
                            </DialogTrigger>
                            <DialogContent className="max-w-4xl h-[80vh] flex flex-col rounded-none border-2 border-black bg-white p-0">
                               ...
                               <div className="p-4 border-b-2 border-black bg-gray-100 flex gap-2">
                                  <Input placeholder="BUSCAR SCRIPT..." value={scriptSearch} onChange={(e) => setScriptSearch(e.target.value)} className="border-black border-2 font-black uppercase text-[11px] rounded-none" />
                               </div>
                                     ...
                                     {filteredScripts.map((s) => (
                                       <div key={s.id} className="p-4 border-2 border-black hover:bg-black group transition-all cursor-default">
                                          <div className="flex justify-between items-start mb-2">
                                            <Badge className="bg-black text-white group-hover:bg-white group-hover:text-black text-[8px] font-black uppercase rounded-none">{s.cat}</Badge>
                                          </div>
                                          <h4 className="font-black uppercase text-[10px] mb-2 group-hover:text-white">{s.title}</h4>
                                          <p className="text-[9px] font-bold text-black/60 group-hover:text-white/60 leading-relaxed uppercase">{s.text.substring(0, 120)}...</p>
                                          <Button onClick={() => handleApplyScript(s.text)} className="w-full mt-4 h-8 bg-white text-black border-2 border-black font-black uppercase text-[8px] rounded-none group-hover:bg-primary group-hover:text-black">Aplicar</Button>
                                       </div>
                                     ))}
                                  </div>
                               </ScrollArea>
                       ...
                       <CardContent className="flex-1 flex flex-col p-4 lg:p-6 space-y-6 min-h-0">
                          <Tabs defaultValue="ia" className="flex-1 flex flex-col min-h-0">
                            <TabsList className="bg-gray-100 border-2 border-black rounded-none p-1 shrink-0 h-12">
                              ...
                              <TabsTrigger value="history" className="flex-1 font-black uppercase text-[9px] data-[state=active]:bg-black data-[state=active]:text-white rounded-none">Histórico Tribunal</TabsTrigger>
                            </TabsList>

                            <TabsContent value="ia" className="flex-1 flex flex-col gap-4 mt-4 min-h-0">
                              <div className="grid grid-cols-3 gap-2 shrink-0">
                                <Button variant="outline" onClick={() => handleGenerateAI('legal')} disabled={isGenerating} className="h-10 border-2 border-black font-black uppercase text-[9px] hover:bg-black hover:text-white transition-all rounded-none bg-white">Jurídico</Button>
                                <Button variant="outline" onClick={() => handleGenerateAI('financeiro')} disabled={isGenerating} className="h-10 border-2 border-black font-black uppercase text-[9px] hover:bg-black hover:text-white transition-all rounded-none bg-white">Financeiro</Button>
                                <Button variant="outline" onClick={() => handleGenerateAI('comercial')} disabled={isGenerating} className="h-10 border-2 border-black font-black uppercase text-[9px] hover:bg-black hover:text-white transition-all rounded-none bg-white">Comercial</Button>
                              </div>
                              <div className="flex-1 bg-[#f8f9fb] border-2 border-dashed border-black/20 p-4 flex flex-col relative min-h-0">
                                {isGenerating && <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex flex-col items-center justify-center z-10"><Loader2 className="animate-spin text-black mb-2" /><p className="text-[9px] font-black uppercase">IA Redigindo...</p></div>}
                                <textarea 
                                  value={aiResponse}
                                  onChange={(e) => setAiResponse(e.target.value)}
                                  placeholder="REDIJA SUA MENSAGEM OU SELECIONE UM MODELO..."
                            ...
                            <TabsContent value="history" className="flex-1 flex flex-col gap-4 mt-4 min-h-0">
                               <Textarea 
                                 placeholder="COLE O HISTÓRICO DE MOVIMENTAÇÕES DO TRIBUNAL AQUI..." 
                                 value={courtHistory}
                               ...
                               <Button onClick={handleGenerateFromHistory} disabled={!courtHistory.trim() || isGenerating} className="w-full bg-black text-white border-2 border-black font-black uppercase h-12 rounded-none">
                                  {isGenerating ? <Loader2 className="animate-spin mr-2" /> : <BrainCircuit size={16} className="mr-2" />}
                                  Analisar Histórico & Gerar Resumo
                               </Button>
                            </TabsContent>
                          </Tabs>
          ...
          {selectedContact && (
            <aside className="hidden xl:flex w-96 border-l-2 border-black bg-white flex flex-col shrink-0">
               <div className="p-6 bg-[#f8f9fb] border-b-2 border-black">
                  <h2 className="text-lg font-black uppercase">{selectedContact.nome}</h2>
                  <p className="text-[10px] font-black uppercase text-black/40">Gabinete de Atendimento Elite</p>
               </div>
               <ScrollArea className="flex-1 p-6">
                  <div className="space-y-6">
                     ...
                     <section className="space-y-3">
                        ...
                        <div className="p-4 bg-[#f3f2f2] border-2 border-black rounded-none">
                           <p className="text-[10px] font-black uppercase leading-relaxed text-black/60 italic">{selectedContact.observacao || 'SEM NOTAS REGISTRADAS.'}</p>
                        </div>
                     </section>
                  </div>
               </ScrollArea>
...
function InfoItem({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) {
  return (
    <div className="flex items-center gap-3">
       <div className="w-7 h-7 bg-[#f3f2f2] border border-black flex items-center justify-center shrink-0">{icon}</div>
       <div className="min-w-0">
